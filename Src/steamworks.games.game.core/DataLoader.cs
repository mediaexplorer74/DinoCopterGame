...
namespace steamworks.games.game.core
{
  public class DataLoader : ICaveCabDataLoader, IDataLoader
  {
    ...
    public TextureInfo[] LoadTextures()
    {
      ...
      Stream stream1 = this.DataUtils.OpenFile("assets\\sprites.xml");
    ...
    {
      CaveLevel Level = new XmlSerializer(typeof (CaveLevel)).Deserialize(this.DataUtils.OpenFile("assets\\levels\\" + this.GetLevelName(levelNumber))) as CaveLevel;
      string mapName = this.GetMapName(levelNumber);
      XmlSerializer xmlSerializer = new XmlSerializer(typeof (TMXMapData));
      Level.CaveMap = xmlSerializer.Deserialize(this.DataUtils.OpenFile("assets\\maps\\" + mapName)) as TMXMapData;
    ...
    public SoundInfo[] LoadSounds()
    {
      ...
      Stream stream1 = this.DataUtils.OpenFile("assets\\sounds.xml");
      Stream stream2 = stream1;
