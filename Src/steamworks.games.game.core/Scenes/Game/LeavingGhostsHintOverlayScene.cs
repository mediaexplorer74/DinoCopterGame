...
// Type: Steamworks.Games.Game.Core.Scenes.Game.LeavingGhostsHintOverlayScene
...
namespace Steamworks.Games.Game.Core.Scenes.Game
{
  public class LeavingGhostsHintOverlayScene : HintScene
  {
    ...
    public LeavingGhostsHintOverlayScene(EngineBase context)
      : base(context)
    {
      this._tutGraphic = context.SpriteFactory.Get("ingametut2");
      this._tutGraphic.CenterX(this.Context.ScreenWidth);
      this._tutGraphic.Y = 150f;
      this.RootEntity.AttachChild(this._tutGraphic);
      this._tutText = context.SpriteFactory.GetText("Land next to a gate!", "mediumfont");
      this._tutText.CenterX(this.Context.ScreenWidth);
      this._tutText.X -= 205f;
      this._tutText.Y = 300f;
